...
crosslet.MapView = (function(_super) {
  ...
  MapView.prototype.initialize = function(el, config) {
    ...
    return this.ds.loadGeo(this.geoURL, this.config.map.geo.id_field, function(ds) {
      ...
      _this.hoverElementRect = _this.hoverElement.append("svg:rect").attr("x", 0).attr("y", 0).attr("width", 10).attr("height", 60).attr("rx", 5).attr("ry", 5);
      _this.hoverElementText1 = _this.hoverElement.append("text").attr("x", 0).attr("y", 0);
      _this.hoverElementText2 = _this.hoverElement.append("text").attr("x", 0).attr("y", 16);
  ...
  MapView.prototype.reset = function() {
    ...
    this.svg.attr("width", topRight[0] - bottomLeft[0] + 200).attr("height", bottomLeft[1] - topRight[1]).style("margin-left", bottomLeft[0] + "px").style("margin-top", topRight[1] + "px");
  ...
  MapView.prototype.mouseMove = function() {
    ...
    pos[0] += 30;
    pos[1] += 30;
    if (this.hoverElementTextBB.width + pos[0] + 100 >= this.svg.attr("width")) {
  ...
  MapView.prototype.hover = function(data) {
    ...
    this.hoverElementText1.text(text[0]);
    this.hoverElementText2.text(text[1]);	
    this.hoverElementTextBB1 = this.hoverElementText1.node().getBBox();
    this.hoverElementTextBB2 = this.hoverElementText2.node().getBBox();
    this.hoverElementTextBB = this.hoverElementText1.node().getBBox();	
	if (this.hoverElementTextBB1.width > this.hoverElementTextBB2.width) {
		this.hoverElementTextBB.width = this.hoverElementTextBB1.width;
	} else {
		this.hoverElementTextBB.width = this.hoverElementTextBB2.width;		
	}
    return this.hoverElementRect.attr("width", this.hoverElementTextBB.width + 10).attr("height", this.hoverElementTextBB.height + 26).attr("x", this.hoverElementTextBB.x - 5).attr("y", this.hoverElementTextBB.y - 5);
  };
...
//global level renderLegend function
function renderLegend(view, mapContainer) {
	
	// remove classed legends with D3
	var removeableLegend = d3.select('.removeableLegend');
	if (!removeableLegend) {
		// do nothing
	} else {
		removeableLegend.remove();
	}
		
	if (view == 'kirwanIndex') {
		
	    var legendColor = L.control({position: 'topright'});

	    legendColor.onAdd = function (map) {
	        var div = L.DomUtil.create('div', 'info legend removeableLegend');  
	        	div.innerHTML += '<img src="assets/images/Kirwan_Index_Scale.png"/>';
	        return div;
	    };

	    legendColor.addTo(mapContainer);
	    ...
	    legendColor.onAdd = function (map) {
	        var div = L.DomUtil.create('div', 'info legend removeableLegend');  
	        	div.innerHTML += '<img src="assets/images/EDU_Index_Scale.png"/>';
	        return div;
	    };
	    ...
	    legendColor.onAdd = function (map) {
	        var div = L.DomUtil.create('div', 'info legend removeableLegend');  
	        	div.innerHTML += '<img src="assets/images/EE_Index_Scale.png"/>';
	        return div;
	    };
	    ...
	    legendColor.onAdd = function (map) {
	        var div = L.DomUtil.create('div', 'info legend removeableLegend');  
	        	div.innerHTML += '<img src="assets/images/N_Index_Scale.png"/>';
	        return div;
	    };
